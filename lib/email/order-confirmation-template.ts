...
export interface OwnerOrderNotificationData extends OrderConfirmationData {
  customerEmail?: string
  customerName?: string
}

export function ownerOrderNotificationHtml(data: OwnerOrderNotificationData): string {
  const {
    orderNumber,
    customerName,
    customerEmail,
    ...
    .map(
      (item) => `
        <tr>
          ...
          <td style="padding:10px 0;border-bottom:1px solid #eee;text-align:right;font-size:14px;color:#1a1a1a;">
            ${money(item.lineTotal)}
          </td>
        </tr>`,
    )
  ...
  const customerLines = [
    customerName ? `<p style="margin:0;font-size:14px;color:#1a1a1a;">${escapeHtml(customerName)}</p>` : '',
    customerEmail ? `<p style="margin:2px 0 0;font-size:14px;color:#555;">${escapeHtml(customerEmail)}</p>` : '',
  ].join('')
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border:1px solid #e0ddd5;">
          ...
          <tr>
            <td style="background:#1a1a1a;padding:24px 40px;text-align:center;">
              <p style="margin:0;font-family:Georgia,serif;font-size:11px;letter-spacing:3px;text-transform:uppercase;color:#c9a96e;">Stone International Wellness Center</p>
              <h1 style="margin:10px 0 0;font-family:Georgia,serif;font-size:20px;font-weight:normal;color:#ffffff;letter-spacing:1px;">New order received</h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:32px 40px;">
              <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e0ddd5;background:#f9f8f5;margin-bottom:24px;">
                <tr>
                  <td style="padding:16px 20px;">
                    <p style="margin:0;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#999;">Order number</p>
                    <p style="margin:4px 0 0;font-family:'Courier New',monospace;font-size:18px;color:#1a1a1a;letter-spacing:2px;">${escapeHtml(orderNumber)}</p>
                  ...
                  <td style="padding:16px 20px;text-align:right;">
                    <p style="margin:0;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#999;">Total paid</p>
                    <p style="margin:4px 0 0;font-size:18px;color:#1a1a1a;font-weight:bold;">${money(total)}</p>
                  </td>
                </tr>
              </table>

              ${customerLines ? `
              <p style="margin:0 0 8px;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#999;">Customer</p>
              <div style="margin-bottom:24px;">
                ${customerLines}
              </div>` : ''}

              <p style="margin:0 0 8px;font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#999;">Items</p>
              <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
                ${itemRows}
              </table>
          ...
          <tr>
            <td style="background:#f9f8f5;border-top:1px solid #e0ddd5;padding:20px 40px;text-align:center;">
              <p style="margin:0;font-size:12px;color:#aaa;">
                Full order details are in Sanity Studio under Orders.
              </p>
            </td>
          </tr>
...
export function ownerOrderNotificationText(data: OwnerOrderNotificationData): string {
  const {
    orderNumber,
    customerName,
    customerEmail,
  ...
  lines.push('', 'Full order details are in Sanity Studio under Orders.')
